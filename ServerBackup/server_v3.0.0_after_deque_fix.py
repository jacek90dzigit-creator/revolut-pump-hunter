...
def _safe_deque_snapshot(dq, retries=10):
    """Read-only stable snapshot of a deque modified by live feed tasks."""
    if not dq:
        return []
...
def get_old_price(history: deque, target_age_seconds: int, now: float) -> Optional[float]:
    ...
    candidate = None
    for ts, price in _safe_deque_snapshot(history):
        if ts <= target_time:
            candidate = price
        else:
            break
...
def activity_metrics(asset: str, now: float) -> Dict[str, object]:
    ...
    for ts, value in _safe_deque_snapshot(history):
...
def _context_price_at_age(asset: str, now: float, age_seconds: int) -> Optional[float]:
    ...
    candidate = None
    for ts, price, _source in _safe_deque_snapshot(history):
        if ts <= target:
            candidate = price
        else:
            break
...
def market_context_metrics(asset: str, current_price: float, now: float) -> Dict[str, object]:
    ...
    path = []
    if history:
        path = [(ts, p) for ts, p, _ in _safe_deque_snapshot(history) if ts >= now - MARKET_CONTEXT_MAX_SECONDS and p > 0]
...
def order_flow_metrics(asset: str, now: float) -> Dict[str, object]:
    ...
    def calc(seconds: int):
        ...
        buy = sell = 0.0
        trades = 0
        for ts, side, value in _safe_deque_snapshot(dq):
            if ts < cutoff:
                continue
            trades += 1
            if side == "buy":
...
def dynamic_momentum_metrics(
    ...
    if history:
        cutoff = now - 600
        path_prices = [p for ts, p in _safe_deque_snapshot(history) if ts >= cutoff and p > 0]
        if path_prices:
...
def _seed_history(asset: str, source: str, source_symbol: str, candles: List[tuple]) -> int:
    if not candles:
        return 0
    history = price_history.setdefault(asset, deque(maxlen=8000))
    existing = {int(ts) for ts, _ in _safe_deque_snapshot(history)}
    added = 0
...
def get_signals(limit: int = 50) -> Dict[str, object]:
    ...
    for raw in _safe_deque_snapshot(signals)[:safe_limit]:
        item = dict(raw)
...
def re_entries(limit: int = 50) -> Dict[str, object]:
    safe_limit = max(1, min(limit, 200))
    rows = [sig for sig in _safe_deque_snapshot(signals) if sig.get("type") == "RE_ENTRY"]
    return {
        "count": len(rows),
...
def signal_engine() -> Dict[str, object]:
    ...
    reentry_count = sum(1 for sig in _safe_deque_snapshot(signals) if sig.get("type") == "RE_ENTRY")
...
def hot_detail(asset: str) -> Dict[str, object]:
    ...
    return {
        ...
        "state": dict(st),
        "snapshots": _safe_deque_snapshot(hot_snapshots.get(symbol)),
    }
...
def hot_report(limit: int = 100) -> Dict[str, object]:
    ...
    return {
        ...
        "completed": _safe_deque_snapshot(completed_hot_cycles)[:safe_limit],
        "rules": {
...
def compact_report(limit: int = 200) -> Dict[str, object]:
    ...
    items = _safe_deque_snapshot(signals)[:safe_limit]
    ...
    return {
        ...
        "attention": {
            ...
            "recent_completed": _safe_deque_snapshot(completed_hot_cycles)[:20],
        },
        "rules": {
...
def signal_detail(index: int) -> Dict[str, object]:
    items = _safe_deque_snapshot(signals)
...
def asset_status(asset: str) -> Dict[str, object]:
    ...
    return {
        ...
        "hot_snapshots": _safe_deque_snapshot(hot_snapshots.get(symbol))[-30:],
